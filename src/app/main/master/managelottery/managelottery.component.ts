...
interface TableRow {
  ...
  storeId:number;
  lotteryName: string;
  lotteryPayType:number;
  visible: boolean;
  storeName:string;
}
@Component({
  selector: 'app-managelottery',
  templateUrl: './managelottery.component.html',
  styleUrls: ['./managelottery.component.scss']
})
export class ManagelotteryComponent {
  ...
  expensehead:any;
   ...
   public form = new FormGroup({
    lotteryTypeId: new FormControl(0),
    lotteryName:new FormControl(''),
    storeId: new FormControl(0, Validators.required),
    lotteryPayType: new FormControl(0, Validators.required),
 ...
 // =============================================================================
   ...
   onSubmit() {
     ...
     this.http.post(environment.SaveLotteryType, this.form.value).subscribe((result: any) => {
       if (result.isSuccess == 1) {
         this.isLoading = false;
   ...
   onEdit(pId:any)
   {
     this.expensehead = this.datalist.filter(((x: { lotteryTypeId: any; }) => x.lotteryTypeId == pId));
     this.form.patchValue({
      lotteryTypeId: this.expensehead[0].lotteryTypeId,
      storeId:  this.expensehead[0].storeId,
      lotteryName:  this.expensehead[0].lotteryName,
      lotteryPayType:this.expensehead[0].lotteryPayType,
   ...
   paymentOptions: { label: string; value: number }[] = [
    ...
    { label: 'Pay In', value: 0 },
    { label: 'Pay Out', value: 1 }
  ];
