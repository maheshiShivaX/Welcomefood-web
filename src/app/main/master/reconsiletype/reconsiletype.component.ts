...
@Component({
  selector: 'app-reconsiletype',
  templateUrl: './reconsiletype.component.html',
  styleUrls: ['./reconsiletype.component.scss']
})
export class ReconsiletypeComponent {
 ...
 // =============================================================================
   ...
   onSubmit() {
     ...
     this.http.post(environment.SaveCashReconsilType, this.form.value).subscribe((result: any) => {
   ...
   onEdit(pId:any)
   {
     this.expensehead = this.datalist.filter(((x: { cashReconsilTypeId: any; }) => x.cashReconsilTypeId == pId));
     this.form.patchValue({
      cashReconsilTypeId: this.expensehead[0].cashReconsilTypeId,
      name:  this.expensehead[0].name,
