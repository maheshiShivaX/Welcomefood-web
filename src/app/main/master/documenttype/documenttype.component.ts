...
export class DocumenttypeComponent {
  ...
  public form = new FormGroup({
    ...
    isExpiredDocument: new FormControl(0)
  });
  ...
  onSubmit() {
    ...
    this.form.patchValue(
      {
        isExpiredDocument : this.isExpire,
      }
    )
    ...
    this.http.post(environment.SaveDocumentType, this.form.value).subscribe((result: any) => {
  ...
  onEdit(pId: any) {
    this.expensehead = this.datalist.filter(((x: { documentTypeId: any; }) => x.documentTypeId == pId));
    this.form.patchValue({
      documentTypeId: this.expensehead[0].documentTypeId,
      documentTypeName: this.expensehead[0].documentTypeName,
      isActive: this.expensehead[0].isActive,
      createdBy: this.expensehead[0].createdBy,
      isExpiredDocument: this.expensehead[0].isExpiredDocument,
    });
    this.isExpire =this.expensehead[0].isExpiredDocument ;
 ;
    this.openPopup('');
  }
  ...
  onDelete(pId: any) {
    ...
    this.http.getAll(environment.DeleteDocumentTypeById + "?pDocumentTypeId=" + pId).subscribe((result: any) => {
      if (result.isSuccess == 1) {
