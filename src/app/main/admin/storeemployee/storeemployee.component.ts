...
@Component({
  ...
  styleUrls: ['./storeemployee.component.scss']
})
export class StoreemployeeComponent {
 ...
 // =============================================================================
   ...
   onSubmit() {
     ...
     this.http.post(environment.SaveStoreEmployee, this.form.value).subscribe((result: any) => {
       if (result.isSuccess == 1) {
         this.isLoading = false;
   ...
   onEdit(pId:any)
   {
     this.expensehead = this.datalist.filter(((x: { employeeId: any; }) => x.employeeId == pId));
     this.form.patchValue({
      employeeId: this.expensehead[0].employeeId,
      storeId:  this.expensehead[0].storeId,
      name:  this.expensehead[0].name,
      emailId:this.expensehead[0].emailId,
      mobileNo:this.expensehead[0].mobileNo,
